...
import {Categories, Products, P_detail} from './Exam2'
//Routes : switch같은 개념으로 가장 구체적인 경로를 우선 매칭한다.(라우터들을 그룹화 하는 컴포넌트라고 생각하면 됨)
//Route : URL과 컴포넌트를 매핑하여 특정 경로에 맞는 컴포넌트를 렌더링 한다.

//Route 컴포넌트의 주요 속성
//1. Path -> URL 경로를 정의한다, 사용자가 특정 경로에 접근할 때 어떤 컴포넌트를 렌더링할지 결정한다.
//2. element -> path에 들어있는 경로와 일치할 때 렌더링할 컴포넌트를 지정한다.
  ...
  return (
    <div className="App">
      ...
      <Routes>
        ...
        <Route path="/categories/:categoryId/products/:productId" element={<P_detail />} />
      </Routes>
    </div>
  );
}
